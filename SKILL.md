...
### Step 1b: AI API Key（Kimi 优先，支持兜底）
...
question({
  questions: [{
    header: "Kimi API Key",
    question: "推荐提供 Kimi API Key 作为主模型（可选再配置 OPENAI_API_KEY 兜底）\n\n获取方式：访问 https://platform.moonshot.cn/console/api-keys 创建免费 API Key",
    options: []
  }]
})
```

如果 `config.kimiApiKey` 已存在，跳过此步。
...
- 至少一个 AI API Key（`KIMI_API_KEY` 或 `OPENAI_API_KEY`）
...
需要提供 Kimi API Key，可在 https://platform.moonshot.cn/console/api-keys 免费获取。
